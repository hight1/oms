...
import { getUploadToken, getHospitalSpecial, queryHospitalSpecial, queryHospitalRegion, queryHospitalCategory, queryHospitalLevel, query, create, update, deny, remove, verify, batchRemove, queryUser, queryHospitalRank } from '../services/members'
...
export default modelExtend(pageModel, {
  ...
  reducers: {
    ...
    updateSuccess (state, action) {
      ...
      const newList = state.list.map((members) => {
        if (members.id === newUser.id) {
          return { ...members, ...newUser }
        }
        return members
      })
    ...
    grantSuccess (state, action) {
      ...
      const newList = state.list.map((members) => {
        if (members.id === grantUser.id) {
          members.roles = grantUser.roles
          return { ...members }
        }
        return members
      })
  ...
  effects: {
    ...
    *create ({ payload }, { call, put, select }) {
      const icon = yield select(({ members }) => members.currentItem.hospitalIcon)
      const img = yield select(({ members }) => members.currentItem.hospitalImg)
    ...
    *update ({ payload }, { call, put, select }) {
      const code = yield select(({ members }) => members.currentItem.code)
      const icon = yield select(({ members }) => members.currentItem.hospitalIcon)
      const img = yield select(({ members }) => members.currentItem.hospitalImg)
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        const match = pathToRegexp('/members').exec(location.pathname)
        if (match) {
